...
// checkLimitsDigitalBridges проверяет ограничения времени и памяти
func checkLimitsDigitalBridges(maxTime time.Duration, maxMemoryMB int, fn func()) {
...
// EdgeDigitalBridges represents a directed edge in the graph
type EdgeDigitalBridges struct {
	to       int
	capacity int
	flow     int
	cost     int64
...
type Graph struct {
	adj [][]EdgeDigitalBridges
}

func NewGraph(n int) *Graph {
	return &Graph{
		adj: make([][]EdgeDigitalBridges, n),
	}
}

func (g *Graph) AddEdge(from, to, cap int, cost int64) {
	forward := EdgeDigitalBridges{to: to, capacity: cap, flow: 0, cost: cost, rev: len(g.adj[to])}
	backward := EdgeDigitalBridges{to: from, capacity: 0, flow: 0, cost: -cost, rev: len(g.adj[from])}
...
// https://coderun.yandex.ru/selections/2025-winter-common/problems/digital-bridges
// DigitalBridges - problem 18
func DigitalBridges() {
	checkLimitsDigitalBridges(2*time.Second, 1024, solveDigitalBridges)
}
